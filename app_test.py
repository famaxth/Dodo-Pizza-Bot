...
def callback_inline(call):
    if call.message:
        ...
        elif call.data == '🖌Дизайн':
            bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text="Любой дизайн под ваши нужды")
            bot.edit_message_reply_markup(chat_id=call.message.chat.id, message_id=call.message.message_id, inline_message_id=call.inline_message_id, reply_markup=mine.oplati)
...
def send_otziv(message):
    if message.text == '💌 Отзывы':
        ...
        bot.send_message(message.chat.id, "📎Контакты:\n\n◼️ Администратор - @por0vos1k\n\nУслуги гаранта(5%) - @Kukol6 ✔️")
    elif message.text == 'Работа с RatHouse':
        print('Нажал Работа. ID: '+str(message.chat.id)+'    Дата/время: '+str(datetime.now()))
        bot.send_message(message.chat.id, "📌Команда RatHouse ищет людей для сотрудничества📌\n\nМы являемся большой организацией для оказания услуг специфической направленности, а следовательно мы ищем людей в нашу команду, которые умеют их оказывать.\n\n🧨 Доступные вакансии:\n\n- Сотрудник банка\n- Деанон, пробив\n- Дизайнер\n- Чёрные ресселеры\n- Сотрудник сотовой связи\n- Программист (Веб-разработка)\n- Представители власти\n- Арбитраж\n- Администраторы\n- Услуги рефа и возврата денег\n\n🪓 Нужные поставщики:\n\n- Техника\n- Алкоголь\n- Сигареты", reply_markup=mine.svize)
        ...
        if message.chat.id == config.admin_id:
            bot.send_message(config.admin_id, "Выберите нужный кошелек:", reply_markup=mine.money_cash)
        ...
        if message.chat.id == config.admin_id:
            bot.send_message(config.admin_id, '🔨Этот раздел еще в разработке')
    elif message.text == '💲Настройка платежек':
        if message.chat.id == config.admin_id:
            bot.send_message(config.admin_id, '🔨Этот раздел еще в разработке')
        ...
        if message.chat.id == config.admin_id:
            bot.send_message(config.admin_id, '🔨Этот раздел еще в разработке')
    elif message.text == '⛔️Черный список':
        if message.chat.id == config.admin_id:
            bot.send_message(config.admin_id, '🔨Этот раздел еще в разработке')
    elif message.text == '🙎‍♂️Пользователи':
        if message.chat.id == config.admin_id:
            file_users = open("joined.txt", "r")
            a = file_users.read()
            bot.send_message(config.admin_id, "Пользователи: {}".format(a))
